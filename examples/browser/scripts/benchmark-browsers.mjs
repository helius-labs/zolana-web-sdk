...
async function prove() {
  ...
  const proveMs = Number(await driver.findElement(By.css(".timing > span")).getText());
  const verifyMs = Number.parseFloat(await driver.findElement(By.css(".verify-time")).getText());
  return { proveMs, verifyMs };
}
try {
  await driver.get(process.env.DEMO_URL || "http://127.0.0.1:5178/");
  await driver.wait(until.elementLocated(By.css("textarea")), 30000);
  await driver.wait(
    async () => (await driver.findElement(By.css("textarea")).getAttribute("value")).length > 100,
    30000,
  );
  results.environment = await driver.executeScript(
  ...
  for (const count of counts) {
    ...
    await driver.findElement(By.css('button[aria-label="Options"]')).click();
    const mode = await driver.findElement(By.id("proving-mode"));
    await mode.findElement(By.css('option[value="custom"]')).click();
    const input = await driver.findElement(By.id("thread-count"));
    ...
    await driver.findElement(By.css('button[aria-label="Close options"]')).click();
    const warmups = [];
    for (let i = 0; i < 3; i++) warmups.push(await prove());
    assert.ok(
      (await driver.findElement(By.css(".proof-facts")).getAttribute("textContent")).includes(
        `Arkworks · ${count} threads`,
      ),
    );
    const samples = [];
    ...
    const run = {
      ...
      warmups,
      samples,
      medianMs: median(samples.map((x) => x.proveMs)),
      verifyMedianMs: median(samples.map((x) => x.verifyMs)),
    };
    results.runs.push(run);
    ...
    console.log(
      JSON.stringify({
        browser: browserName,
        count,
        firstMs: warmups[0].proveMs,
        medianMs: run.medianMs,
        verifyMedianMs: run.verifyMedianMs,
      }),
    );
  }
} finally {
